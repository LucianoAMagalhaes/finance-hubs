...
import { estadoVazio, PERCENTUAIS_PADRAO, projetarMes, type Percentuais } from "@/dominio";
...
const PADRAO = rascunhoDe(projetarMes(estadoVazio(), "2026-09").potes);
...
describe("o rascunho dos percentuais", () => {
  ...
  it("um rascunho válido devolve os percentuais lidos, sem erro, e a soma; espaços em volta não atrapalham", () => {
    expect(conferirRascunho({ ...PADRAO, metas: " 5 " })).toEqual({
      percentuais: { ...PERCENTUAIS_PADRAO, metas: 5 },
      erro: null,
      soma: 90,
    });
  });
  ...
  it("acima de 100 ou negativo é recusado; na soma e na prévia, entra limitado a 100 ou a 0", () => {
    ...
    expect(percentuaisParaPrevia(passou)).toEqual<Percentuais>({ ...PERCENTUAIS_PADRAO, "custos-fixos": 100, prazeres: 0 });
  ...
  it("a prévia conta como zero o que não se lê como número", () => {
    expect(percentuaisParaPrevia({ ...PADRAO, metas: "", conhecimento: "x" })).toEqual<Percentuais>({
      ...PERCENTUAIS_PADRAO,
      metas: 0,
      conhecimento: 0,
    });
